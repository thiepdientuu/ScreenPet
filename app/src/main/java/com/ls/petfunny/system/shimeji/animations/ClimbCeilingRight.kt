...
class ClimbCeilingRight(shimejiId: Int, paidenable: Boolean, flinging: Boolean) : ClimbCeiling(shimejiId, paidenable, flinging) {
    ...
    override fun getSprites(): MutableList<Sprite> {
        ...
        return when (shimejiId) {
            32 -> {
                ...
                spritesAkari.add(Sprite(SpriteUtil.CLIMB_CEILINGAKARI.values[2], 1, 0, 20))
                spritesAkari
            }
            116 -> {
    ...
    override val nextAnimation: Animation
        get() =
            if (classicmode) {
                if (ThreadLocalRandom.current().nextBoolean()) DescendRight(shimejiId, paidenable = paidenable, flinging = flinging) else ClimbCeilingLeft(
                    shimejiId,
                ...
                when (ThreadLocalRandom.current().nextInt(1, 5)) {
                    1 -> DescendRight(shimejiId, paidenable = paidenable, flinging = flinging)
                    2 -> ClimbCeilingLeft(shimejiId, paidenable = paidenable, flinging = flinging)
                    3 -> ClimbCeilingLeft(shimejiId, paidenable = paidenable, flinging = flinging)
